...
const CancellationMode = () => {
  ...
  useEffect(() => {
    const fetchContent = async () => {
      try {
        ...
        setTextContent(res?.data?.data?.cancellation_policy || "");
      } catch (error) {
  ...
  // Save updated cancellation policy
  const handleSave = async () => {
    try {
      setIsSaving(true);
      await apiClient.put(`/admin/cancellation-policy`, {
        cancellation_policy: textContent,
      });
    ...
    <div className="flex flex-col gap-y-8 p-4 md:p-8">
      ...
      <div>
        ...
        <p className="mt-1 text-sm text-slate-600 dark:text-slate-400">
          View, edit, and manage the general cancellation policy.
        </p>
      </div>
        ...
        <div className="mt-6 min-h-[300px]">
          {isLoading ? (
            <div className="text-center py-20 flex flex-col items-center justify-center">
              ...
              <p className="text-sm text-slate-500 dark:text-slate-400">Loading Policy...</p>
            </div>
          ) : isEditing ? (
            <textarea
              value={textContent}
